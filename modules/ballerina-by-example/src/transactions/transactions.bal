...
function main (string[] args) {
    ...
    //errors etc. The retry count is the number of times the transaction
    //is tried before aborting. By default a transaction is tried three
    ...
    transaction with retries(4){
        ...
        //Anytime the transaction can be forcefully aborted using the abort
        //keyword
        if (c == 0) {
            abort;
        }
        ...
        //The failed block will be executed if the transaction is failed due
        //to an exception or a throw statement. This block will execute each
        //time transaction is failed until retry count is reached.
