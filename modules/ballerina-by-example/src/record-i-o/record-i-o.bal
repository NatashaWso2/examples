...
function getFileRecordChannel (string filePath, string permission, string encoding,
    ...
    //Then we convert the byte channel to character channel to read
    //content as text.
    io:CharacterChannel characterChannel = channel.toCharacterChannel(encoding);
    //Finally we convert the character channel to record channel to read
    //content as records.
...
//This function will process CSV file and write content back as text
//with '|' delimiter
function process (io:TextRecordChannel srcRecordChannel,
                  io:TextRecordChannel dstRecordChannel) {
    int numberOfFields = -1;
    //We read all the records from the provided file until there're no
    // records returned
...
function main (string[] args) {
    ...
    //Here we specify the location of the CSV file where the record
    //separator is new line and field separator is comma.
    io:TextRecordChannel srcRecordChannel =
    getFileRecordChannel(srcFileName, "r", "UTF-8", "\\r?\\n", ",");
    //Here we specify the location of the text file where the record
    //separator is new line and field separator is pipe.
    io:TextRecordChannel dstRecordChannel =
